...
- The `scope` option scopes all selector queries to `containerRef.current`, so `.box` only matches elements *inside* that container — safe to reuse across multiple instances.
...
**Live result — scroll into view to trigger:**
