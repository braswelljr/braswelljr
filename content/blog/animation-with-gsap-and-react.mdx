...
These are the three animation primitives. The difference is simply where the animation _starts_ and _ends_.
...
- The `scope` option scopes all selector queries to `containerRef.current`, so `.fade-box` only matches elements _inside_ that container — safe to reuse across multiple instances.
...
| --------------- | ---------------------------------------- |
| `'-=0.2'`       | 0.2s before the _previous_ tween ends    |
| `'+=0.1'`       | 0.1s after the _previous_ tween ends     |
| `'<'`           | Same start time as the _previous_ tween  |
| `'<0.1'`        | 0.1s after the _previous_ tween _starts_ |
...
**Always scope with a container ref.** Class-based selectors like `.box` are global. Without `{ scope: containerRef }`, your animation may accidentally target elements in _other_ mounted components.
