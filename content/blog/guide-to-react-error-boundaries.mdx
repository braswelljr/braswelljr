...
React has no built-in error boundary component — not in React 18, and not in React 19. The most popular way to get a functional-friendly API is the [`react-error-boundary`](https://github.com/bvaughn/react-error-boundary) library, which provides a ready-made `<ErrorBoundary>` component you can drop in:

```tsx title="react-error-boundary-lib.tsx"
...
```tsx title="react-19-root-handlers.tsx"
...
```tsx title="using-error-boundary.tsx"
import * as React from 'react';
...
```tsx title="functional-error-handler.tsx" twoslash
import * as React from 'react';
...
```tsx title="error-boundary-placement.tsx"
import * as React from 'react';
import ErrorBoundary from './ErrorBoundary';
...
```tsx title="data-fetching-protection.tsx"
import * as React from 'react';
import ErrorBoundary from './ErrorBoundary';
...
```tsx title="third-party-protection.tsx"
import * as React from 'react';
...
Either way, remember there is **no** built-in error boundary component in React — including React 19. What React 19 _does_ add is root-level `onCaughtError` / `onUncaughtError` callbacks for centralized logging.
